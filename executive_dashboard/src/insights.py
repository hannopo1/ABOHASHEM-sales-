...
def generate(kpis, customers, products, receivables, monthly, dq, focus_month=None) -> dict:
    ...
    why_aging = ("معظم الرصيد «جاري» (غير مستحق) وهو مؤشر صحي." if overdue_share < 0.4
                 else "غالبية الرصيد متأخرة السداد — يُخصَّص الرصيد النهائي على الفواتير بطريقة "
                      "FIFO (الأقدم سدادًا أولًا)، وكل فاتورة قبل يوليو ما تزال غير مسددة تُصنَّف متأخرة.")
    out["aging"] = _insight(
        "أعمار الديون (FIFO على الرصيد النهائي)",
